...
bool gyroInitComplete = false;
float yawCompensationGain=0.0;
void setup()
{
  Wire.begin();
  bAutoStart=true;
...

{
  while (fifoCount < packetSize) 
  {
    ...
    //Imu initialize Check
    if((millis()>12000) && !gyroInitComplete)
    {
      gyroInitComplete = true;
      stewartFeedback.sta =stewartFeedback.sta| (gyroInitComplete<<4);
      gyroCompensationActive=true;
      stewartFeedback.sta =stewartFeedback.sta| (gyroCompensationActive<<3);
      yawZero=-ypr[0]*180/PI;//Degrees
    }

    //Check if new lynx data has been received
    if (updateInfo.state == LynxLib::eNewDataReceived)
    {
      if(stewartControl.command!=0)
      {
        if((stewartControl.command == 10) && gyroInitComplete)
        {
          gyroCompensationActive = true;
          stewartFeedback.sta =stewartFeedback.sta | (1<<3);
          yawZero=-ypr[0]*180/PI;//Degrees
        }
        ...
        if((stewartControl.command == 12))
        {
          yawCompensationGain = 1.5;
          stewartFeedback.sta =stewartFeedback.sta | (1<<2);
          
        }
        if(stewartControl.command == 13)
        {
          yawCompensationGain = 0.0;
          stewartFeedback.sta = stewartFeedback.sta & ~(1<<2);
        }
      }
    }//endif
    ...
    if(gyroCompensationActive)
      AlphaCalc(stewartControl.setpointX,stewartControl.setpointY,stewartControl.setpointZ+12.0,1.5*stewartFeedback.imuPitch*pi/180.0,1.5*stewartFeedback.imuRoll*pi/180.0,yawCompensationGain*(stewartFeedback.imuYaw*pi/180.0));
    else
      AlphaCalc(stewartControl.setpointX,stewartControl.setpointY,stewartControl.setpointZ+12.0, stewartControl.setpointRoll*pi/180.0, stewartControl.setpointPitch*pi/180.0, stewartControl.setpointYaw*pi/180.0);
...
if(gyroCompensationActive)
{
  ...
  stewartFeedback.feedbackRoll = 1.5*stewartFeedback.imuPitch;
      stewartFeedback.feedbackPitch = 1.5*stewartFeedback.imuRoll;
      stewartFeedback.feedbackYaw = yawCompensationGain * stewartFeedback.imuYaw;
}
else
{
...
else 
{
  ...
  else 
  {
    ...
    mpu.dmpGetYawPitchRoll(ypr, &q, &gravity);
      ...
      stewartFeedback.imuYaw = -ypr[0]*180/PI - yawZero; //Degrees
